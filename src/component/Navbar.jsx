      ...
      <div className='w-full flex justify-between items-start max-w-7xl mx-auto'>
        ...
        >
          ...
          <p className='text-white text-[18px] font-bold cursor-pointer hidden sm:block'>
            <span style={{ fontSize: '35px !important' }}>A</span>
            <span className='text-orange-600'>Bendaoui</span>
          </p>
        </Link>
              ...
              {navLinks.map((link) => {
                return (
                  <li
                    ...
                    className={`${
                      active === link.id ? 'text-white' : 'text-secondary'
                    } hover:text-white text-[16px] font-medium cursor-pointer duration-500`}
                  >
                    <a
                      href={`#${link.id}`}
                      onClick={() => {
